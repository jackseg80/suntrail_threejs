    ...
    for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
        try {
            const resp = await fetch(url, {
                headers: {
                    'User-Agent':
                        'SunTrail3D-BuildScript/1.0 (one-time tile bundling)',
                },
            });
            ...
            console.warn(
                `  Retry ${attempt}/${MAX_RETRIES} for ${ref.z}/${ref.x}/${ref.y}`
            );
...
async function main() {
    ...
    for (const layer of LAYERS) {
        const count = refs.filter(
            (r) => layer.zooms.includes(r.z) && r.source === layer.source
        ).length;
    ...
    for (const ref of refs) {
        ...
        if (done % 50 === 0 || done === totalCount) {
            ...
            const sizeMB = (
                tileBuffers.reduce((s, t) => s + t.data.length, 0) /
                1024 /
                1024
            ).toFixed(1);
    ...
    const metadata = Buffer.from(
        JSON.stringify({
            ...
            description:
                'LOD 5-7 Europe (OpenTopoMap) + LOD 8-11 Suisse (OTM + SwissTopo)',
            format: 'webp',
