...
| `build-country-pack.ts` | Generates a PMTiles package for a country or region. Uses the same polygon filter as the app (`isTileInCountry` runtime). | `npx tsx scripts/build-country-pack.ts --pack <id> --maptiler-key <key>` |
