# SunTrail — Roadmap produit révisée (v5.91.4 + correctifs non publiés)

> Révision produit : 2026-09-20. La source reste `5.91.4` / Android `917`, complétée par un lot non
> publié validé sur S23 : terrain 2D/3D stable, solaire sans exagération visuelle, états REC lisibles,
> chevrons sur toute trace, édition pendant le guidage, points `1…n`, packs et imports GPX renforcés.
> Aucun nouveau tag, release, déploiement Pages ou téléversement Play n'est revendiqué ; le statut et
> le maximum de versionCode de la Play Console restent à vérifier séparément.
...
>
> **À contrôler — boussole 3D** : le canvas `#compass-canvas` a disparu lors de la refonte UI 5.90 ;
> le rendu 3D de la boussole (`compass.ts`) est donc inerte. Décider de restaurer l'élément ou de
> retirer proprement le sous-système. Le bouton « nord » de la carte reste fonctionnel et indépendant.
>
> **À contrôler — pack Suisse** : le pipeline lossless et l'échantillon S23 sont validés, mais le
> candidat complet v5 pèse 1,99 Go. Publication NO-GO avant réduction et validation du vrai trajet
> téléchargement/import OPFS ; le catalogue de production reste inchangé.
...
- **v5.90.0** / Android **911** clôt la refonte UI : système commun de feuilles et dialogues,
  ...
  le chantier de pente du chemin reste un lot séparé, non engagé ici.
- **v5.90.1** / Android **912** relève la luminosité de la 3D en heure dorée : la scène reste proche
  du plein jour quand le soleil est bas mais encore levé, avec la même luminosité au zénith. Le
  réglage suit l'altitude solaire, donc identique au lever et au coucher ; aucune release ni upload
  Play n'est revendiqué.
- **v5.91.0** / Android **913** refond la lecture des pentes du profil et le cadrage des tracés : les
  pentes sont dessinées en bandes façon Openrunner (échelle unique montée/descente, plat discret,
  progression vers le rouge puis le brun) dont la granularité suit le terrain, et la caméra cadre le
  tracé selon ses dimensions réelles, le FOV et le ratio d'écran. La même révision relance
  automatiquement l'analyse solaire du parcours dès que le relief devient disponible, y compris au
  passage 2D → 3D. La release GitHub `v5.91.0` produit l'AAB signé ; aucun upload Play n'est
  revendiqué.
- **v5.91.1** / Android **914** fiabilise l'analyse solaire du parcours et la lecture du profil sur
  petit écran : la fiche ne se bloque plus faute de relief, les ombres inconnues sont hachurées, le
  relief est préchargé le long du tracé (indépendamment de la vue, désactivé en Économie) et les
  informations du profil passent en grille courte `Dist. · Alt. · Pente · Heure`. Le raycast est borné
  pour supprimer les blocages lors de la manipulation de l'heure. La release GitHub `v5.91.1` produit
  l'AAB signé ; aucun upload Play.
- **v5.91.2** / Android **915** rend la trace lisible sur toutes les cartes : couleur unique réglable
  (magenta par défaut, palette dans Réglages) avec casing deux tons, coloration solaire de la trace
  devenue optionnelle, double voie et chevrons pour les allers-retours. Le profil affiche ses
  informations en une ligne et la section « Couleur trace » est compacte. La release GitHub `v5.91.2`
  produit l'AAB signé ; aucun upload Play.
- **v5.91.3** / Android **916** recolore la trace d'enregistrement en vert fluo (au-dessus de son
  casing) et réserve l'inclinomètre au mode 3D, où les pixels d'altitude du relief sont disponibles :
  masqué en 2D, mention « (3D) » dans les réglages. Le repli de pente basé sur le tracé est retiré ;
  le préchargement du relief pour l'analyse solaire reste en place. La release GitHub `v5.91.3`
  produit l'AAB signé ; aucun upload Play.
- **v5.91.4** / Android **917** corrige le démarrage carte : la lecture `CacheStorage` par tuile
  quitte le thread principal (mesurée jusqu'à ~2,9 s par tuile sur S23, > 10 s de couverture sur
  A53) au profit du Web Worker, avec priorité pack OPFS/hors ligne conservée et un repli local borné
  (8 s) sur un miss couleur. L'hydratation de l'UI secondaire est différée après la première tuile.
  La même révision retire deux réglages avancés historiques (debug sans cible et normalmap RG
  désormais toujours actif) et des références DOM mortes. Contrôles S23 et A53 positifs ; la release
  GitHub `v5.91.4` produit l'AAB signé, aucun upload Play.
- **Post-v5.91.4 non publié** stabilise les transitions 2D/3D et le recyclage des shaders terrain,
  aligne l'analyse solaire sur l'altitude physique, distingue REC actif/terminé/guidage, généralise
  les chevrons, permet de modifier le parcours pendant le guidage et aligne les points `1…n`. Le lot
  renforce aussi packs/GPX et sépare validation de `main` et déploiement Pages. Validé sur S23.
...
| **v5.90.1 — correctif local**        | Relever la luminosité 3D en heure dorée                                    | Bonus d'éclairage lié à l'altitude solaire : scène proche du plein jour au soleil bas, identique au lever et au coucher et luminosité du zénith inchangée. APK diagnostic installé sur S23 ; aucune release ni upload Play.                                                                                             |
| **v5.91.0 — clôturée**               | Mieux lire l'effort en montée comme en descente                            | Profil de pente en bandes de couleur façon Openrunner (échelle unique montée/descente, plat discret puis rouge/brun), granularité qui suit le terrain, et cadrage caméra adapté aux dimensions du tracé, au FOV et au ratio d'écran. Release GitHub `v5.91.0` avec AAB signé ; aucun upload Play.                       |
| **v5.91.1 — clôturée**               | Rendre l'analyse solaire du parcours fiable et lisible                     | Fiche non bloquante, relief préchargé le long du tracé (désactivé en Économie), ombres inconnues hachurées, raycast borné (fin des blocages en manipulant l'heure) et infos du profil en grille courte `Dist. · Alt. · Pente · Heure`. Release GitHub `v5.91.1` avec AAB signé ; aucun upload Play.                     |
...
| **Post-v5.91.4 — validé localement** | Stabiliser terrain, parcours et données locales                            | Isolation des offsets shader, transitions 2D/3D sûres, solaire physique, REC actif/terminé distinct, chevrons généralisés, édition pendant guidance, points `1…n`, packs/GPX renforcés. APK diagnostic validé S23 ; pas de nouvelle release ni d'upload Play.                                                           |
