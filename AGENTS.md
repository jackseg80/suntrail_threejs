# SunTrail - Base de Connaissance (v5.19.6)
...
- **OnboardingTutorial v2 (v5.19.6)** : `src/modules/onboardingTutorial.ts` — module standalone (pas un BaseComponent). 8 slides : navigation 3D, recherche, boutons FAB (grille 2×2), parcours GPX/REC (grille verticale), simulation solaire, météo, outils d'analyse (grille verticale 3 items), sécurité & offline. Deux exports : `requestOnboarding()` (flag `suntrail_onboarding_v2` — 1er lancement seulement) et `showOnboarding()` (toujours afficher, pour le bouton Réglages). Appelé dans `ui.ts` via `requestAcceptance().then(() => requestOnboarding())`. `z-index: 9000` (en dessous de l'acceptance wall à 9998). Responsive : `@media (min-width: 768px)` agrandit la carte (520px), les polices et les icônes pour desktop. **Ne jamais appeler `localStorage.setItem(ONBOARDING_KEY)` dans `showOnboarding()`** — uniquement dans `requestOnboarding()` après résolution. Pour forcer un réaffichage global : bumper le suffixe du storage key (`v2` → `v3`).
...
| UI minuscule après rotation paysage→portrait | **Cause racine (v5.20)** : `renderer.setSize(w,h)` posait `canvas.style.width` en pixels paysage. Au retour portrait le canvas débordait (pas d'`overflow:hidden`) → WebView zoomait out toute la page. Les fix v5.16.8/v5.19.1 (manipulation `<meta viewport>`) aggravaient le problème. | **3 correctifs** : (1) `#canvas-container { overflow: hidden }`. (2) `#canvas-container canvas { width:100%!important; height:100%!important }`. (3) `renderer.setSize(w,h,false)` partout. **⚠️ RÈGLES : ne JAMAIS utiliser `setSize()` sans `false` en 3e param. Ne JAMAIS toucher `<meta viewport>` en JS.** |
...
| UI minuscule après rotation paysage→portrait (récidive v5.19.1) | Voir entrée v5.20 ci-dessus — les tentatives JS (viewport meta, retries, visualViewport) ne peuvent pas corriger un zoom WebView déjà committé | Le seul fix fiable est d'empêcher le canvas de déborder via CSS (`overflow:hidden` + `width:100%!important`) + `setSize(w,h,false)`. Le polling orientationchange dans `ui.ts` est un filet de sécurité, pas le fix principal. |
