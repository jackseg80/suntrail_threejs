...
function pseudoRandom(x: number, y: number, seed: number = 0): number {
    const val =
        Math.sin(x * 12.9898 + y * 78.233 + seed * 37.11) * 43758.5453123;
 ...
 */
export async function createForestForTile(
    tile: Tile
): Promise<THREE.Group | null> {
    const isSatellite =
        state.MAP_SOURCE === 'satellite' || state.MAP_SOURCE === 'ign-ortho';
    if (
        !state.SHOW_VEGETATION ||
        isSatellite ||
        !tile.colorTex ||
        !tile.pixelData ||
        tile.zoom < 14
    ...
    if (!scanCanvas) {
        ...
        scanCanvas.width = scanRes;
        scanCanvas.height = scanRes;
    ...
    if (tile.colorScale < 1.0) {
        ctx.drawImage(
            img,
            img.width * tile.colorOffset.x,
            img.height * tile.colorOffset.y,
            img.width * tile.colorScale,
            img.height * tile.colorScale,
            0,
            0,
            scanRes,
            scanRes
    ...
    const step = hasVectors
        ? Math.max(2, state.PERFORMANCE_PRESET === 'eco' ? 4 : 2)
    ...
    const targetTrees = Math.max(
        20,
        Math.floor(state.VEGETATION_DENSITY * areaRatio)
    );
    ...
    const placementProbability = Math.min(
        1.0,
        ((hasVectors ? 2.0 : 1.0) * targetTrees) / totalSlots
    );
    ...
    for (let py = 0; py < scanRes; py += step) {
        for (let px = 0; px < scanRes; px += step) {
            ...
            const spx = Math.floor(
                px + pseudoRandom(globalX, globalY, 1) * step
            );
            const spy = Math.floor(
                py + pseudoRandom(globalX, globalY, 2) * step
            );
            const i =
                (Math.min(scanRes - 1, spy) * scanRes +
                    Math.min(scanRes - 1, spx)) *
                4;
            ...
            if (forests) {
                ...
                if (state.MAP_SOURCE === 'opentopomap') {
                    ...
                    const isNeutral =
                        Math.abs(r - g) < 10 && Math.abs(g - b) < 10 && r > 160;

                    const i_r =
                        (Math.min(scanRes - 1, spy) * scanRes +
                            Math.min(scanRes - 1, spx + 1)) *
                        4;
                    const i_d =
                        (Math.min(scanRes - 1, spy + 1) * scanRes +
                            Math.min(scanRes - 1, spx)) *
                        4;
                    const variance =
                        Math.abs(g - colorData[i_r + 1]) +
                        Math.abs(g - colorData[i_d + 1]);
            ...
            if (isForest) {
                if (pseudoRandom(globalX, globalY, 3) > placementProbability)
                    continue;

                const jx =
                    (pseudoRandom(globalX, globalY, 4) - 0.5) *
                    (size / scanRes) *
                    step;
                const jz =
                    (pseudoRandom(globalX, globalY, 5) - 0.5) *
                    (size / scanRes) *
                    step;
                const lx = (px / scanRes - 0.5) * size + jx;
                const lz = (py / scanRes - 0.5) * size + jz;

                const h = getSimpleAltitude(
                    tile,
                    lx,
                    lz,
                    exaggeration,
                ...
                const scale =
                    (0.35 + pseudoRandom(globalX, globalY, 7) * 0.65) *
                    densityBoost;
                dummy.scale.set(
                    scale,
                    scale * (0.85 + pseudoRandom(globalX, globalY, 8) * 0.45),
                    scale
                );
    ...
    Object.keys(instances).forEach((type) => {
        ...
        if (data.count > 0) {
            ...
            const mat = is2D
                ? essences[type].material2D
                : essences[type].material;
            const iMesh = new THREE.InstancedMesh(
                essences[type].geometry,
                mat,
                data.count
            );

            // v5.36.0 : Injection directe du buffer optimisé
            iMesh.instanceMatrix.array.set(
                data.buffer.subarray(0, data.count * 16)
            );
