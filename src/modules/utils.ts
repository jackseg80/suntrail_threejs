...
const OVERPASS_DELAY = 1200;       
const OVERPASS_BASE_BACKOFF = 15000;  
...
async function processNextOverpass() {
    ...
    // FIFO : on prend le premier (qui est soit un HighPriority unshifted, soit le plus vieux normal)
    ...
    try {
        const response = await fetch(`${server}?data=${encodeURIComponent(item.query)}`, {
            signal: AbortSignal.timeout(30000) // Augmenté à 30s pour laisser le serveur répondre
        });
        ...
        if (response.status === 429 || response.status === 504) {
            ...
            // Disjoncteur après 6 échecs au lieu de 3
            if (_overpassConsecutiveFails > 6) {
                console.warn("[Overpass] Trop d'échecs. Désactivation Hydrologie/Bâtiments pour 2 min.");
                _lowPriorityDisabledUntil = Date.now() + 120000;
            }
            ...
            // Attendre 2s avant de retenter sur le serveur suivant (au lieu de 500ms)
            setTimeout(processNextOverpass, 2000);
            isOverpassProcessing = false;
            return;
        }
