...
// Le zoom de référence pour le monde 3D reste TOUJOURS 13 (pour la précision du GPX)
const WORLD_ZOOM = 13;
...
// Projection vers le monde 3D (Ancrée sur WORLD_ZOOM = 13)
export function lngLatToWorld(lon, lat) {
    const tileSizeRef = EARTH_CIRCUMFERENCE / Math.pow(2, WORLD_ZOOM);
    const xf = (lon + 180) / 360 * Math.pow(2, WORLD_ZOOM);
    const yf = (1 - Math.log(Math.tan(lat * Math.PI / 180) + 1 / Math.cos(lat * Math.PI / 180)) / Math.PI) / 2 * Math.pow(2, WORLD_ZOOM);
    return { 
        x: (xf - (state.originTile.x + 0.5)) * tileSizeRef, 
        z: (yf - (state.originTile.y + 0.5)) * tileSizeRef 
    };
}
...
export function worldToLngLat(worldX, worldZ) {
    ...
    const lon = xf / Math.pow(2, WORLD_ZOOM) * 360 - 180;
    const n = Math.PI - 2 * Math.PI * yf / Math.pow(2, WORLD_ZOOM);
...
export async function updateVisibleTiles(camLat, camLon, camAltitude, worldX, worldZ) {
    ...
    // Le zoom réel utilisé pour charger les données (slider)
    const dataZoom = state.ZOOM; 
    
    const centerTile = lngLatToTile(camLon || state.TARGET_LON, camLat || state.TARGET_LAT, dataZoom);
    
    // Ajustement dynamique du range pour ne pas saturer le GPU au Zoom 15
    let range = state.RANGE;
    if (dataZoom >= 15) range = Math.min(range, 2); 
    ...
    for (let dy = -range; dy <= range; dy++) {
        for (let dx = -range; dx <= range; dx++) {
            ...
            const key = `tile_${dataZoom}_${tx}_${ty}`;
            neededKeys.add(key);
            if (!activeTiles.has(key)) loadTile(tx, ty, dataZoom, key);
        }
    }
...
async function loadTile(tx, ty, zoom, key) {
    ...
    try {
        ...
        // Coordonnées GPS du coin Nord-Ouest de la tuile
        ...
        for (let i = 0; i < vertices.length / 3; i++) {
            ...
            const px = u * 255, py = (1.0 - v) * 255;
            const x0 = Math.floor(px), y0 = Math.floor(py), x1 = Math.min(254, x0+1), y1 = Math.min(254, y0+1);
            ...
            const vLat = latNW - ( (1.0 - v) * (latNW - tileToLat(ty+1, zoom)) );
