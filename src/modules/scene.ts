...
export async function disposeScene(): Promise<void> {
    if (state.controls) {
        if (currentThrottledUpdate)
            state.controls.removeEventListener(
                'change',
                currentThrottledUpdate
            );
        if (currentThrottledSunUpdate)
            state.controls.removeEventListener(
                'change',
                currentThrottledSunUpdate
            );
    }
...
function getIdealZoom(dist: number, currentZoom: number): number {
    ...
    const getThresh = (base: number, z: number) => {
        if (currentZoom === z) return base * 1.1;
        return base;
    };
...
export async function initScene(): Promise<void> {
    ...
    const throttledUpdate = throttle(() => {
        ...
        if (
            clamped.x !== state.controls.target.x ||
            clamped.z !== state.controls.target.z
        ) {
        ...
        const distToLastWeather = haversineDistance(
            gpsCenter.lat,
            gpsCenter.lon,
            state.lastWeatherLat,
            state.lastWeatherLon
        );
        if (distToLastWeather > 3)
            debouncedFetchWeather(gpsCenter.lat, gpsCenter.lon);
        ...
        if (distFromOrigin > 35000) {
            ...
            if (
                ...
                if (!isNaN(newTile.x) && !isNaN(newTile.y)) {
                    ...
                    if (
                        Math.abs(offsetX) < 250000 &&
                        Math.abs(offsetZ) < 250000
                    ) {
                        ...
                        if (state.recordedMesh) {
                            state.recordedMesh.geometry.translate(
                                offsetX,
                                0,
                                offsetZ
                            );
    ...
    eventBus.on(
        'flyTo',
        ({ worldX, worldZ, targetElevation, targetDistance }) => {
    ...
    function updateAutoTilt(distToTarget: number): boolean {
        ...
        if (state.IS_2D_MODE || state.ZOOM <= 10) {
            ...
            if (Math.abs(currentTilt - desiredTilt) > 0.01) {
                const newTilt = THREE.MathUtils.lerp(
                    currentTilt,
                    desiredTilt,
                    0.07
                );
                state.controls.minPolarAngle = Math.max(0.05, newTilt - 0.01);
                state.controls.maxPolarAngle = Math.min(
                    tiltCap,
                    newTilt + 0.01
                );
                return true;
            ...
            const hFactor = THREE.MathUtils.clamp(
                (distToTarget - 2000) / 100000,
                0,
                1
            );
            let desiredTilt = THREE.MathUtils.lerp(
                tiltCap * 0.95,
                0.05,
                Math.pow(hFactor, 0.4)
            );
            if (state.ZOOM <= 11)
                desiredTilt = Math.min(desiredTilt, tiltCap * 0.9);
    ...
    function updateTerrainPhysics(interacting: boolean): void {
        ...
        if (!state.isFlyingTo && !state.isFollowingUser) {
            const targetGroundH = state.IS_2D_MODE
                ? 0
                : getAltitudeAt(
                      state.controls.target.x,
                      state.controls.target.z
    ...
    const renderLoopFn = () => {
        if (!state.renderer || !state.camera || !state.scene || !state.controls)
            return;
        ...
        const interacting = state.isUserInteracting;
        const distToTarget = state.camera.position.distanceTo(
            state.controls.target
        );
        ...
        if (needsUpdate) {
            ...
            if (fpsTick - fpsLastTime >= 1000) {
                state.currentFPS = Math.round(
                    (fpsFrameCount * 1000) / (fpsTick - fpsLastTime)
                );
                fpsFrameCount = 0;
                fpsLastTime = fpsTick;
