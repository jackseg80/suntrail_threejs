...
import {
    ICON_CLOSE, ICON_PLAY, ICON_PAUSE,
    ICON_STOP, ICON_RECORD,
    ICON_CHECK, ICON_LOCK, ICON_UNLOCK, ICON_INFO,
} from './icons';

function parseSvg(svg: string): { width: number; height: number; viewBox: string } | null {
...
const allIcons = [ICON_CLOSE, ICON_PLAY, ICON_PAUSE, ICON_STOP, ICON_RECORD, ICON_CHECK, ICON_LOCK, ICON_UNLOCK, ICON_INFO];
const iconNames = ['ICON_CLOSE', 'ICON_PLAY', 'ICON_PAUSE', 'ICON_STOP', 'ICON_RECORD', 'ICON_CHECK', 'ICON_LOCK', 'ICON_UNLOCK', 'ICON_INFO'];
...
describe('icons.ts — SVG constants', () => {
    ...
    it('chaque icône est une chaîne SVG valide (ouvrante et fermante)', () => {
        allIcons.forEach((icon, i) => {
            expect(icon.startsWith('<svg'), `${iconNames[i]} devrait commencer par <svg`).toBe(true);
            expect(icon.endsWith('</svg>'), `${iconNames[i]} devrait finir par </svg>`).toBe(true);
        });
    });
    ...
    it('chaque icône a un viewBox 0 0 24 24 (taille standard)', () => {
        allIcons.forEach((icon, i) => {
            ...
            expect(parsed, `${iconNames[i]} devrait avoir un viewBox`).not.toBeNull();
            expect(parsed!.viewBox, `${iconNames[i]} viewBox`).toBe('0 0 24 24');
        });
    });
    ...
    it('chaque icône utilise stroke="currentColor" ou fill="var(--danger)"', () => {
        allIcons.forEach((icon, i) => {
            ...
            expect(hasStroke || hasDangerFill, `${iconNames[i]} devrait utiliser une couleur dynamique`).toBe(true);
        });
    });
