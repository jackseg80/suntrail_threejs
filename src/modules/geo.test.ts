...
import { lngLatToWorld, worldToLngLat, lngLatToTile, getTileBounds, isPositionInSwitzerland, isPositionInFrance, isPositionInItaly, haversineDistance, isPointInPolygon, isPointInCountry, isTileInCountry, isTileInSwitzerland, isTileInSwitzerlandStrict } from './geo';
...
describe('Module Géo (geo.ts)', () => {
    ...
    describe('Geographical Detection', () => {
        ...
        it('should reject coordinates outside Switzerland', () => {
            ...
            expect(isPositionInSwitzerland(47.90, 7.25)).toBe(false); // Issenheim (Alsace) — v5.40.50
        ...
        it('should correctly identify French continental coordinates', () => {
            ...
            expect(isPositionInFrance(47.90, 7.25)).toBe(true); // Issenheim (Alsace)
        });
    ...
    describe('Tile-in-Country (multi-point check)', () => {
        ...
        it('isTileInCountry should return false for a tile fully outside CH', () => {
            ...
            expect(isTileInCountry(tile.x, tile.y, tile.z, 'CH', 3)).toBe(false);
            expect(isTileInCountry(tile.x, tile.y, tile.z, 'CH', 5)).toBe(false);
        });

        it('isTileInSwitzerland vs isTileInSwitzerlandStrict for Issenheim border', () => {
            // Issenheim (~47.90°N, ~7.25°E) — à la frontière CH/FR
            const tile12 = lngLatToTile(7.25, 47.90, 12);
            const tile14 = lngLatToTile(7.25, 47.90, 14);

            // Ni LOD 12 ni LOD 14 ne devraient être classifiés CH
            expect(isTileInSwitzerland(tile12.x, tile12.y, 12)).toBe(false);
            expect(isTileInSwitzerlandStrict(tile12.x, tile12.y, 12)).toBe(false);
            expect(isTileInSwitzerland(tile14.x, tile14.y, 14)).toBe(false);
            expect(isTileInSwitzerlandStrict(tile14.x, tile14.y, 14)).toBe(false);
        });
