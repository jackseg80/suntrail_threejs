...
function renderBuildingsPBF(tile: Tile, buildings: any[]) {
    ...
    const offX = rtx * ratio - tile.tx,
        offY = rty * ratio - tile.ty;
    ...
    const maxBuildings = inCH
        ? Math.min(500, state.BUILDING_LIMIT * 3)
        : state.BUILDING_LIMIT;
    ...
    buildings.forEach((feat) => {
        ...
        const processed: THREE.Vector2[][] = rings.map((r: any[]) =>
            r.map(
                (p) =>
                    new THREE.Vector2(
        ...
        if (processed[0].length > 2) {
            try {
                ...
                const h =
                    (typeof levels === 'number' ? levels : parseFloat(levels)) *
                    3.5 *
                    state.RELIEF_EXAGGERATION;

                geometries.push(
                    createBuildingManualGeometry(
                        processed,
                        h,
                        10 * state.RELIEF_EXAGGERATION,
                        baseAlt
                    )
                );
                count++;
                ...
                if (state.DEBUG_MODE)
                    console.warn('[Buildings] Geometry failed', e);
            }
        }
    });
    ...
    if (geometries.length > 0) {
        try {
            ...
            const mesh = new THREE.Mesh(
                merged,
                state.IS_2D_MODE ? buildingMaterial2D : buildingMaterial
            );
            mesh.castShadow =
                !state.IS_2D_MODE &&
                state.PERFORMANCE_PRESET !== 'eco' &&
                state.BUILDINGS_SHADOWS;
            mesh.receiveShadow = !state.IS_2D_MODE;

            if (tile.buildingGroup && state.scene)
                state.scene.remove(tile.buildingGroup);
            ...
            console.warn('[Buildings] Merge failed', e);
        } finally {
...
function createBuildingManualGeometry(
    rings: THREE.Vector2[][],
    height: number,
    skirt: number,
    baseAlt: number
    ...
    if (rings.length > 0 && rings[0].length >= 3) {
        try {
            ...
            if (state.DEBUG_MODE)
                console.warn('[Buildings] ShapeGeometry failed', e);
        }
    }
