...
function doPan(dx: number, dy: number): void {
    ...
    const height =
        (_canvas as HTMLCanvasElement).clientHeight || window.innerHeight;
    const scale =
        (PAN_SPEED * (2 * dist * Math.tan((_camera.fov * Math.PI) / 360))) /
        height;
...
function doZoom(ratio: number): void {
    ...
    _camera.position
        .copy(_controls.target)
        .addScaledVector(_zoomDir.normalize(), newDist);
}
...
function tickInertia(): void {
    _velX *= INERTIA;
    _velY *= INERTIA;
    if (Math.abs(_velX) < 0.1 && Math.abs(_velY) < 0.1) {
        _velX = 0;
        _velY = 0;
        return;
    }
...
function onPointerMove(e: PointerEvent): void {
    ...
    if (_pointers.size === 1) {
        ...
        const is2D =
            !!_controls && _controls.minPolarAngle === _controls.maxPolarAngle;
        ...
        if (_tiltLocked) {
            if (absDy > 0.3) void (is2D ? doPan(0, -dy) : doTilt(dy));
        } else {
            const isRotating =
                absDAngle > ROT_DEADZONE &&
...
function onPointerUp(e: PointerEvent): void {
    ...
    if (_pointers.size === 0) {
        if (
            ...
            (Math.abs(_velX) > 0.5 || Math.abs(_velY) > 0.5)
        ) {
            _inertiaId = requestAnimationFrame(tickInertia);
        }
        if (_controls) {
