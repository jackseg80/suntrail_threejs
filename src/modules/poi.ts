...
function getPOITexture(category: POICategory): THREE.Texture {
    ...
    if (ctx) {
        ...
        if (category === 'guidepost') {
            ...
            // @ts-ignore roundRect not in CanvasRenderingContext2D types
            if (ctx.roundRect) {
                // @ts-ignore roundRect not in CanvasRenderingContext2D types
...
export async function loadPOIsForTile(tile: Tile) {
    if (
        !state.SHOW_SIGNPOSTS ||
        tile.zoom < state.POI_ZOOM_THRESHOLD ||
        (tile.status as string) === 'disposed'
    )
        return;
...
async function fetchPOIsWithCache(
    ...
    try {
        ...
        layers.forEach((layerName) => {
            ...
            const isTransportName =
                lowerLayer.includes('transport') && lowerLayer.includes('name');

            // Filtrage des couches de géométrie (polygones/lignes) — on ignore sauf transportation_name
            if (
                !isTransportName &&
                (lowerLayer.includes('line') ||
                    lowerLayer.includes('poly') ||
                    lowerLayer.includes('water') ||
                    lowerLayer.includes('landuse') ||
                    lowerLayer.includes('building') ||
                    lowerLayer.includes('transport') ||
                    lowerLayer.includes('road') ||
                    lowerLayer.includes('highway'))
            )
                return;
            ...
            for (let i = 0; i < layer.length; i++) {
                ...
                if (!category) {
                    ...
                    if (
                        (cls === 'lodging' &&
                            ['alpine_hut', 'wilderness_hut'].includes(sub)) ||
                        cls === 'alpine_hut' ||
                        cls === 'wilderness_hut' ||
                    ...
                    // Haltes (pique-nique, campings, eau potable)
                        ...
                        (cls === 'tourism' &&
                            ['picnic_site', 'camp_site'].includes(sub)) ||
                        (cls === 'lodging' && sub === 'camp_site') ||
                        (cls === 'amenity' &&
                            ['drinking_water', 'fountain'].includes(sub)) ||
                        cls === 'picnic_site' ||
                        cls === 'camp_site' ||
                    ...
                    // Curiosités naturelles
                        ...
                        (cls === 'natural' &&
                            ['waterfall', 'cave_entrance'].includes(sub)) ||
                        cls === 'waterfall' ||
                        cls === 'cave_entrance' ||
                        props.natural === 'cave_entrance' ||
                        props.waterway === 'waterfall'
                    ) {
                        category = 'attraction';
                    }
                    // Points de vue
                    ...
                    // Guideposts
                        ...
                        props.information === 'guidepost' ||
                        raw.includes('guidepost') ||
                        raw.includes('signpost')
                    ) {
                        category = 'guidepost';
                    }
                }
