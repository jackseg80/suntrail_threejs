...
describe('IAPService - Blindage (v5.29.36)', () => {
    ...
    it("doit s'initialiser correctement en natif", async () => {
        ...
        expect(mockPurchases.configure).toHaveBeenCalledWith({
            apiKey: 'mock-sdk-key-1234567890',
        });
    ...
    it("syncProStatus doit accorder l'accès si entitlement actif", async () => {
        mockPurchases.getCustomerInfo.mockResolvedValue({
            customerInfo: {
                entitlements: { active: { 'SunTrail 3D Pro': {} } },
            },
        });
        await iapService.initialize();
    ...
    it("syncProStatus doit garder l'état actuel en cas d'erreur réseau", async () => {
        ...
        mockPurchases.getCustomerInfo.mockRejectedValue(
                ...
                availablePackages: [
                    {
                        ...
                        packageType: 'ANNUAL',
                        product: { identifier: 'prod_annual' },
                    },
                ],
            },
        });
        ...
        mockPurchases.getOfferings.mockResolvedValue({
            current: {
                availablePackages: [
                    ...
                    {
                        identifier: 'annual',
                        product: { priceString: '29,99 € for 5 minutes' },
                    },
                    {
                        identifier: 'lifetime',
    ...
    it('purchasePack doit chercher dans toutes les offerings', async () => {
        ...
        mockPurchases.getOfferings.mockResolvedValue({
            all: {
                packs: {
                    availablePackages: [
                        {
                            identifier: 'suntrail_pack_switzerland',
                            product: {
                                identifier: 'suntrail_pack_switzerland',
                            },
                        },
        ...
        mockPurchases.purchasePackage.mockResolvedValue({
            customerInfo: {
                entitlements: { active: { 'SunTrail Pack Switzerland': {} } },
            },
        });
    ...
    it('restorePurchases doit gérer les échecs', async () => {
        await iapService.initialize();
        mockPurchases.restorePurchases.mockRejectedValue(
