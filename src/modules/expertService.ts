...
export class ExpertService {
    ...
    generateWeatherReport(wd: NonNullable<typeof state.weatherData>): string {
        const lines = [
            ...
            `${i18n.t('weather.stat.comfortIndex')}: ${Math.round(getComfortIndex(wd.temp, wd.windSpeed, wd.uvIndex ?? 0, wd.humidity, wd.precProb ?? 0, wd.windGusts, wd.hourly?.[0]?.code, wd.visibility, wd.cloudCover) * 10) / 10}/10`,
        ];
