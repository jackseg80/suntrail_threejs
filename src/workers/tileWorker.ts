...
self.onmessage = async (e: MessageEvent<TileWorkerRequest>) => {
    ...
    try {
            ...
            if (elevRes.bitmap) {
                ...
                if (ctx) {
                    ...
                    if (!is2D) {
                        ...
                        if (nCtx) {
                            nCtx.putImageData(
                                new ImageData(normalData, width, height),
                                0,
                                0
                            );
                            const normalBitmap =
                                nCanvas.transferToImageBitmap();
        ...
        if (colorRes) {
            ...
            if (colorRes.fromCache) results.cacheHits++;
            else results.networkRequests++;
            if (colorRes.bitmap) {
        ...
        if (overlayRes) {
            ...
            if (overlayRes.fromCache) results.cacheHits++;
            else results.networkRequests++;
