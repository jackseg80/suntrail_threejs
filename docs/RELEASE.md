...
| **773** | **5.40.27** | **v5.40.27** | **Production (SwissTopo Z14 + High Res Buildings Fix)** | **2026-04-28** |
...
| **911 (release GitHub ; Play à vérifier)** | **5.90.0** | **v5.90.0** | **Refonte UI validée A53/S23 : tutoriel concret et aides contextuelles, surfaces unifiées Free/Pro, rotation paysage sans bandes blanches et commandes latérales homogènes. AAB signé produit par GitHub Actions ; aucun upload Play.** | **2026-09-13** |
| **912 (préparé localement ; Play non attribué)** | **5.90.1** | **v5.90.1** | **Luminosité 3D relevée en heure dorée (réglage lié à l'altitude solaire, zénith inchangé) et analyse solaire d'un point clarifiée : frise sous le graphique, légende, ombre bleu-gris, curseur glissant, phases unifiées dans `solarPhases.ts`. APK diagnostic installé sur S23 ; aucune release GitHub ni upload Play produit.** | **2026-09-15** |
| **913 (release GitHub ; Play à vérifier)** | **5.91.0** | **v5.91.0** | **Profil de pente en bandes de couleur façon Openrunner (échelle unique montée/descente, plat discret puis rouge/brun), granularité qui suit le terrain, cadrage caméra adapté aux dimensions du tracé, au FOV et au ratio d'écran, et relance automatique de l'analyse solaire du parcours dès que le relief arrive (fin du blocage « données de relief non chargées »). AAB signé produit par GitHub Actions ; aucun upload Play.** | **2026-09-16** |
| **914 (release GitHub ; Play à vérifier)** | **5.91.1** | **v5.91.1** | **Analyse solaire du parcours fiable : fiche non bloquante, relief préchargé le long du tracé (désactivé en Économie), ombres inconnues hachurées, raycast borné (fin des freezes en manipulant l'heure) et infos du profil en grille courte `Dist. · Alt. · Pente · Heure`. AAB signé produit par GitHub Actions ; aucun upload Play.** | **2026-09-16** |
| **915 (release GitHub ; Play à vérifier)** | **5.91.2** | **v5.91.2** | **Trace lisible sur toutes les cartes : couleur unique réglable (magenta par défaut) + casing deux tons, coloration solaire de la trace optionnelle, double voie + chevrons pour les allers-retours, profil en une ligne et réglages compacts. AAB signé produit par GitHub Actions ; aucun upload Play.** | **2026-09-16** |
| **916 (release GitHub ; Play à vérifier)** | **5.91.3** | **v5.91.3** | **Trace REC en vert fluo au-dessus de son casing ; inclinomètre réservé au mode 3D (masqué en 2D, mention « (3D) » dans les réglages) et repli de pente basé sur le tracé retiré, sans toucher au préchargement du relief pour l'analyse solaire. AAB signé produit par GitHub Actions ; aucun upload Play.** | **2026-09-16** |
| **917 (release GitHub ; Play à vérifier)** | **5.91.4** | **v5.91.4** | **Démarrage carte accéléré : lecture `CacheStorage` déplacée dans le Web Worker (repli local borné à 8 s sur un miss couleur) et hydratation de l'UI secondaire différée après la première tuile. Retrait des réglages avancés historiques (Infos de debug sans cible, Normalmap RG Compact désormais toujours actif) et de références DOM mortes. AAB signé produit par GitHub Actions ; aucun upload Play.** | **2026-09-17** |
