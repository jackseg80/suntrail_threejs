...
/**
 * RecordingService — Foreground Service Android avec GPS natif (v5.28.1)
 *
 * Architecture Single Source of Truth (v5.25.0):
 * - Ce service est la SEULE source d'enregistrement GPS
 * - Les points sont stockés dans Room SQLite
 * - Le JS lit uniquement via RecordingPlugin qui query la base Room
 * - Filtrage complet (v5.28.1) : (0,0), précision, altitude, vitesse, distance, timestamps, jumps
 */
public class RecordingService extends Service {
    ...
    private static final float  MAX_ALT_DELTA_M     = 200.0f;      // v5.28.1: Rejet sauts altitude
    ...
    // Immobility (Auto-Pause)
    private long    mStartTime                 = 0;
    private Location mLastSignificantLocation  = null;
    private long    mLastMovementTime          = 0;
    private boolean mIsImmobile                = false;
    private static final float  IMMOBILITY_DISTANCE_THRESHOLD = 3.0f; // 3m
    private static final long   IMMOBILITY_TIME_THRESHOLD     = 10000L; // 10s (v5.28.1)
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
            ...
            if (mCurrentCourseId != null) {
                mDbExecutor.execute(() -> {
                    try {
                        ...
                        Log.e(TAG, "Error fetching pointCount: " + e.getMessage());
                    }
                });
            } else {
                mCurrentCourseId = java.util.UUID.randomUUID().toString();
        ...
        getSharedPreferences("RecordingPrefs", MODE_PRIVATE).edit()
            ...
            .putLong("startTime", mStartTime).apply();
        ...
        mLocationCallback = new LocationCallback() {
            ...
            public void onLocationResult(@NonNull LocationResult result) {
                for (Location loc : result.getLocations()) {
                    // v5.28.1: Rejet (0,0)
                    if (Math.abs(loc.getLatitude()) < 0.0001 && Math.abs(loc.getLongitude()) < 0.0001) {
                        continue;
                    }
                    ...
                    // v5.28.1: Filtrage saut altitude
                    if (mLastValidLocation != null) {
                        ...
                        if (Math.abs(altOrthometric - lastAlt) > MAX_ALT_DELTA_M) {
                            Log.d(TAG, "REJECT: altitude jump " + Math.abs(altOrthometric - lastAlt) + "m");
                            continue;
                        }
                    }
                    ...
                    if (mLastValidLocation != null) {
                        ...
                        if (distance2D < MIN_DISTANCE_M) {
                            updateImmobilityStatus(loc); // Check pause even if point rejected for distance
                            continue;
                        }

                        double altDiff = altOrthometric - (mLastValidLocation.getAltitude() - estimateGeoIdHeight(mLastValidLocation.getLatitude(), mLastValidLocation.getLongitude()));
                    ...
                    if (mPointBuffer.size() >= BATCH_SIZE || (now - mLastBatchFlush) > BATCH_FLUSH_INTERVAL_MS) {
                        flushBuffer(now, newCount);
                    } else {
                        if (sCallback != null) sCallback.onNewPoints(mCurrentCourseId, newCount, mIsImmobile);
                    }
    ...
    private void flushBuffer(long now, int newCount) {
    ...
    private void setupNotificationIntents() {
        ...
        if (mStopReceiver == null) {
            ...
            registerReceiver(mStopReceiver, new IntentFilter(STOP_ACTION), Context.RECEIVER_NOT_EXPORTED);
        }
    }

    private void configureGpsRequest(Intent intent) {
        SharedPreferences prefs = getSharedPreferences(PREFS_NAME, MODE_PRIVATE);
        long interval = (intent != null) ? intent.getLongExtra("interval", 3000L) : prefs.getLong("interval", 3000L);
        float minD = (intent != null) ? intent.getFloatExtra("minDisplacement", 0.5f) : prefs.getFloat("minDisplacement", 0.5f);
        boolean highAcc = (intent != null) ? intent.getBooleanExtra("highAccuracy", true) : prefs.getBoolean("highAccuracy", true);

        mLocationRequest = LocationRequest.create()
            .setInterval(interval)
            .setFastestInterval(1000L)
            .setSmallestDisplacement(minD)
            .setPriority(highAcc ? Priority.PRIORITY_HIGH_ACCURACY : Priority.PRIORITY_BALANCED_POWER_ACCURACY);
    }
    ...
    public void onDestroy() {
        ...
        if (!mPointBuffer.isEmpty()) flushBuffer(System.currentTimeMillis(), mPointCount.get());
    ...
    private void updateImmobilityStatus(Location currentLocation) {
        ...
        if (distance > IMMOBILITY_DISTANCE_THRESHOLD) {
            ...
            if (now - mLastMovementTime > IMMOBILITY_TIME_THRESHOLD) {
                mIsImmobile = true;
            }
        }
    }
    ...
    private String getElapsedTimeString() {
        long elapsedMins = (System.currentTimeMillis() - mStartTime) / 60000L;
        return (elapsedMins / 60) > 0 ? (elapsedMins / 60) + "h " + (elapsedMins % 60) + "min" : elapsedMins + "min";
    }

    private Notification buildNotification(int pointCount) {
        String text = getElapsedTimeString() + " — " + pointCount + " points";
        String title = mIsImmobile ? "⚠️ Immobile — Toujours actif ?" : "SunTrail 3D — Enregistrement actif";
        return new NotificationCompat.Builder(this, CHANNEL_ID)
            .setContentTitle(title).setContentText(text)
            .setSmallIcon(R.mipmap.ic_launcher).setContentIntent(mOpenPendingIntent)
            .setOngoing(true).setPriority(mIsImmobile ? NotificationCompat.PRIORITY_DEFAULT : NotificationCompat.PRIORITY_LOW)
            .setCategory(NotificationCompat.CATEGORY_SERVICE)
            .addAction(android.R.drawable.ic_delete, "Arrêter REC", mStopPendingIntent)
            .build();
    }
    ...
    private void updateAdaptiveGpsConfig() {
        ...
        long newInterval = (mCurrentSpeedMps < SPEED_WALKING_SLOW || mIsImmobile) ? 10000 : (mCurrentSpeedMps < SPEED_WALKING_FAST ? 5000 : 3000);
        int newPriority = (mCurrentSpeedMps < SPEED_WALKING_SLOW || mIsImmobile) ? Priority.PRIORITY_BALANCED_POWER_ACCURACY : Priority.PRIORITY_HIGH_ACCURACY;
